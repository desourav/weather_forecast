...
const getAllData = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (currentWeatherProps != undefined) {
            ...
            currWindspeed = currentWeatherProps.windSpeed.value == null ? "unknown" : currentWeatherProps.windSpeed.value.split("to").map((s) => Math.round(parseInt(s.split("km/h")[0].trim()) / 1.6)).join(" to ") + " mph";
        ...
        for (let i = 0; i < forecastArray.length; i++) {
            let dailyWeather = {
                ...
                "windSpeed": forecastArray[i].windSpeed.split("to").map((s) => Math.round(parseInt(s.split("km/h")[0].trim()) / 1.6)).join(" to ") + " mph",
